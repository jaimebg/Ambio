...
Audio files are CC0 (public domain). Per-sound provenance — Freesound id,
uploader and licence URL — is recorded in [`audio-src/sources.json`](audio-src/sources.json).
The white and brown noise beds are generated by `tools/synth-noise.sh` rather
than recorded, so they are public domain by construction.
